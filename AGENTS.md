...
- `src/components/`: `ui/` (primitivos), `layout/` (Header/Footer), `common/` (recursos compartilhados como `Logo` e `GlyphMatrix`).
...
- Design tokens: cores e fontes em `src/index.css` dentro do bloco `@theme` (`--color-purple-*`, `--font-display`, `--font-mono`). Use `var(--color-*)` e `color-mix()` para variantes com alpha em vez de hex solto.
